...
=======
  ...
  $("#WebCrawlCaller").submit(function(e) {
    ...
    var settings = {
      ...
      crossDomain: true,
      url:
        "https://127.0.0.1:54321/crawl?key=78942ef2c1c98bf10fca09c808d718fa3734703e",
      method: "GET",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded"
      },
      ...
      rep.forEach(element => {
        element.forEach(element2 => {
          for (let index = 0; index < element2.length; index++) {
            $("#dump").append(
              JSON.stringify(element2[index]).substring(
                0 + 1,
                JSON.stringify(element2[index]).length - 1
              ) + "\n"
            );
          }
        });
    ...
    for (let j = 0; j < rep.length; j++) {
      ...
      for (let index = 0; index < rep[j].length / 4; index++) {
        ...
        for (
          ...
          if ($("#data tr:last").children()[index2].value == "") i++;
          if (i >= 4) {
            $("#data tr:last").remove();
          }
        }
  ...
  $("#postdata").click(function(e) {
    ...
    for (let element = 0; element < $("#data").children().length; element++) {
      ...
      for (let input = 0; input < 4; input++) {
        if (
          $("#data").children()[element].children[input].children[0] !=
          undefined
        ) {
          switch (pattern.val().charAt(input)) {
            case "d":
              date = $("#data").children()[element].children[input].children[0]
                .value;
              break;
            case "t":
              title = $("#data").children()[element].children[input].children[0]
                .value;
              break;
            case "b":
              des = $("#data").children()[element].children[input].children[0]
                .value;
              break;
            case "p":
              place = $("#data").children()[element].children[input].children[0]
                .value;
              break;
            default:
              break;
          }
            var data = `data ${element}, ${input}`
            json.push({
                id: data,
                data: $("#data").children()[element].children[input].children[0]
                .value
            });
        }
      }
...
>>>>>>> 77e8111fb4dba746b63eef295d42af0d7991b6f5
